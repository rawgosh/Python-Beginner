...
def button_press(num): #defining a function for each button pressd
    global equation_text #creating a global variable for the function to run
    
    equation_text = equation_text + str(num) #adds the pressed number
    equation_label.set(equation_text) #changes the label 

def equals():
    global equation_text 
    try:
        total = str(eval(equation_text)) #evaluates the passed equation
...
def clears():
    ...
    equation_label.set("") #clears the label
...
equation_label = StringVar() #creating a string variable 

label = Label(window, textvariable=equation_label,font=("consolas",20),bg="white",width=24, height = 2) #creating a label to hold the pressed number as a equation
label.pack()

frame = Frame(window) #creating a frame
frame.pack()
...
product = Button(frame, text = "*", height = 4, width = 9, font = 35, command =lambda: button_press("*"))
...
clear = Button(window, text = "Clear", height = 4, width = 15, font = 35, command = clears)
clear.pack()
